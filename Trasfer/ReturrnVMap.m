...
perpV = deltaV; % perpendicular velosity
...
M  = TimeToMeanAnomaly(mu0,R1,phi_rot,T); % Initial phase of target planet
...
deltaV = linspace(Vmin,Vh,10);
parV = Vh - deltaV; % paralel velocity after burn
...
M = phi1+TimeToMeanAnomaly(mu0,R1,phi1,-1.*T); % Initial phase of target planet
M0 = TimeToMeanAnomaly(mu0,R2,0,T); % End phase of source planet
