...
W = 2*pi./(6*3600); % Kerbin angular velocity
...
for it=2:numel(t)
    ...
    v(it) = -1*dt*g + dt*0.5*ro*exp(-1*h(it-1)/H)*(v(it-1)^2)*d*a + dt*(R+h(it-1))*(w(it-1))^2 + v(it-1);
    w(it) = -dt*0.5*ro*exp(-1*h(it-1)/H)*(((R+h(it-1))*w(it-1))^2)*d*a/(R+h(it-1)) + w(it-1);
...
title(sprintf('\\phi = %2.1f\\circ, \\Delta\\phi = %2.1f\\circ,',p(p_idx)*180/pi, W*t(p_idx)*180/pi ));
grid on;